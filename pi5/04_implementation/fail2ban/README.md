...
- host-level log-driven intrusion response layer
...
In this lab, Fail2Ban is not treated as a standalone security product. It is one defensive layer sitting beside SSH, Nextcloud, nginx, mail notifications, and the rest of the host monitoring setup.

What interacts with what

- SSH authentication events come from the system journal
- Nextcloud and nginx events come from application log files on the host
- jail filters evaluate those sources and raise ban decisions
- ban actions trigger both firewall action and email visibility
...
- email notifications were added so bans are visible without having to inspect the host manually
- recidive is enabled so repeated abuse is treated differently from one short burst of failures

It is a practical setup for a single-node system: simple enough to operate, but still tied into the parts of the stack that matter most.

Constraints that shaped it

- single host, so the response layer has to live where the logs already converge
- application logs are split across journal and bind-mounted service logs
- the setup needs to stay maintainable without turning into a full SIEM-style pipeline
- response is reactive, so log quality and path correctness matter a lot
